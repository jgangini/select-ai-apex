...
from .demo_data import demo_entry_for_schema, render_demo_schema_ddl
...
def uses_demo_schema(options: DeploymentOptions, schema: str) -> bool:
    return options.mode == "new" and demo_entry_for_schema(schema) is not None


def effective_owner(options: DeploymentOptions, owner: str) -> str:
    entry = demo_entry_for_schema(owner)
    return entry.schema if options.mode == "new" and entry else owner.upper()
...
def bundled_demo_schemas(options: DeploymentOptions) -> list[str]:
    demo_schemas: list[str] = []
    for owner in options.schemas:
        if uses_demo_schema(options, owner):
            demo_schema = effective_owner(options, owner)
            if demo_schema not in demo_schemas:
                demo_schemas.append(demo_schema)
    for obj in options.tables:
        if uses_demo_schema(options, obj.owner):
            demo_schema = effective_owner(options, obj.owner)
            if demo_schema not in demo_schemas:
                demo_schemas.append(demo_schema)
    return demo_schemas


def render_demo_schema_setup(options: DeploymentOptions, demo_schema: str) -> str:
    entry = demo_entry_for_schema(demo_schema)
    if not entry:
        raise KeyError(f"Unknown bundled demo schema: {demo_schema}")
    return render_demo_schema_ddl(entry, app_schema=options.app_schema, demo_password=options.apex_password)


def render_demo_schema_setups(options: DeploymentOptions) -> str:
    return "\n\n".join(render_demo_schema_setup(options, demo_schema) for demo_schema in bundled_demo_schemas(options))


def render_grants(options: DeploymentOptions) -> str:
    app_schema = sql_identifier(options.app_schema)
    statements: list[str] = []
    demo_setups = render_demo_schema_setups(options)
    if demo_setups:
        statements.append(demo_setups)
...
def render_app_sql(options: DeploymentOptions) -> str:
    ...
    legacy_profile_cleanup = ""
    if options.profile_name.upper() != "GROK_REASONING":
        legacy_profile_cleanup = """
BEGIN
  DBMS_CLOUD_AI.DROP_PROFILE(profile_name => 'GROK_REASONING', force => TRUE);
EXCEPTION
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('WARN legacy profile cleanup skipped: ' || SQLERRM);
END;
/
"""
...
def render_report(options: DeploymentOptions, installed_application_id: int | None = None) -> str:
    object_scope = object_list_json(options)
    dsn = options.dsn or "(selected at runtime)"
    app_id = str(installed_application_id or options.apex_application_id or "generated by APEX")
...
When `--mode new` targets a bundled demo listed in `data/manifest.json`, the generated ADMIN SQL creates the repository-owned demo schema from JSON metadata under `data/<folder>/data`.
Seed rows are loaded from the matching CSV files by the Python loader with array/batch inserts; no static `install.sql` or bulk `INSERT` script is versioned.
