...
When `--schemas HR,SH` is supplied for an existing database, the generated admin SQL grants current tables, views, and materialized views in those schemas to `SELECT_AI_APP`. If ADMIN cannot grant another owner's objects, `--schema-passwords-file` connects as each explicitly selected owner and performs the same object-level grants. The password map is read from a private temporary file, is never placed directly in command arguments, and is not copied into reports or generated secrets. The Select AI profile uses owner-level `object_list` entries so future tables can be added by running more object grants and recreating the profile.
